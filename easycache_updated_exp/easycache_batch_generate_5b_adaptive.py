...
def _mode_config(mode_name: str) -> Dict:
    ...
    elif mode_name.startswith("cog_ec_adaptive_hi"):
        # e.g. cog_ec_adaptive_hi0.10_lo0.075_f9_l6
        parts = mode_name.split("_")
        hi_val = float(parts[3].replace("hi", ""))
        lo_val = float(parts[4].replace("lo", ""))
        f_steps = int(parts[5].replace("f", ""))
        l_steps = int(parts[6].replace("l", ""))
        return {
            "mode": "easycache",
            "thresh_high": hi_val,
            "thresh_low": lo_val,
...
def main() -> None:
    ...
    for local_i, p in enumerate(prompts_slice):
        ...
        prompt_text = p.get("prompt_en", p.get("prompt", str(p)))
        ...
        for mode_name in mode_names:
            ...
            out_path = mode_dir / f"{prompt_text}-{args.seed}.mp4"
